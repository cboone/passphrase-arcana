...
The list is designed for use with [`phraze`][phraze] and [other passphrase generators](#the-word-list) that can use custom lists. At 26,126 words it provides 14.7 bits of entropy per word.
...
[`passphrase-arcana.txt`](./passphrase-arcana.txt) contains 26,126 lowercase ASCII words, ready for use with `phraze`:
...
The scores for the 26,126 words in the final list range from a minimum of 1.25 to a median of 1.95 and a maximum of 2.5. Lower is easier to type. About 32% of words score below 1.8 (easy range), and 55% below 2.0.
...
For this list (26,126 words), each word in your passphrase counts for 14.7 bits of entropy. A 6-word passphrase provides:
...
```math
\text{guessing entropy} = \frac {(2 ^ {88.0} \text{ bits} + 1 \text{ bit})} {2 \text{ bits} / \text{guess}}
```
...
| [Entropy per word](#how-passphrase-entropy-works)       | 14.673 bits     |
| [Efficiency per character](#efficiency-per-character)   | 1.947 bits      |
| [Shortest edit distance](#edit-distance)                | 1               |
| [Mean edit distance](#edit-distance)                    | 7.097           |
