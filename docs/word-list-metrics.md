...
Entropy per word divided by mean word length. Higher values mean more entropy per keystroke. This list's efficiency of 1.99 bits/character means a 50-character passphrase (about 6 words with spaces) provides ~89 bits of entropy, compared to ~50 bits from a random string of 50 lowercase letters (which has ~4.7 bits per character but is much harder to remember).
